...
class SlackNotification(ANotification):
    ...
    def send_message(self, json_data: dict, client : dict) -> bool:
        sucess : bool = True
        logging.info(f"Slack Data: {json_data} Client: {client}")

        request_tx = {
            "text" : f"[{json_data['from']}]: {json_data['content']}"
        }
        m1 = time.monotonic()
        try :
                ...
                try:
                    ...
                    if self.url != 'None':
                        ...
                        if response is not None and response.status_code == 200:
                            ...
                            logging.info("No se notifica nada por Slack")
